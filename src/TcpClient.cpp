...
void TcpClient::start() {
  errno = 0;
  int rv =
      connect(_fd, reinterpret_cast<sockaddr *>(&_sockAddr), sizeof(_sockAddr));
  if (rv < 0) {
    _error = "connect()";
  }
...
int TcpClient::fd() const { return _fd; }

sockaddr_in TcpClient::sockAddr() const { return _sockAddr; }
